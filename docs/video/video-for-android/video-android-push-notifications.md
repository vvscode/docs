...
If the payload that was forwarded to the Sinch client was for an instant message, call `SinchClient.startListeningOnActiveConnection()` which will make sure the `onIncomingMessage` callback is called. See [Active connection](#section-active-connection) for more information.
