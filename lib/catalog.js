...
const { checkUrlExists } = require('./tools');
...
class Link extends Element {
    ...
    static findAll(page) {
        ...
        while ((match = findLinks.exec(sources)) !== null) {
            ...
            if (text.startsWith('!')) {
                types = [LocalImage, RemoteImage];
            } else {
                types = [UrlLink, MailtoLink, XrefLink];
            }

            var linkClass = types.find(type => type.matches(href));

            if (linkClass === undefined) {
                console.warn(`Link [${href}] does not correspond to a supported type of link.`);
                continue;
            }
            links.push(new linkClass(page, text, match.groups, positionIndex));
        }
        return links;
    }
}
...
class XrefLink extends Link {
    static matches(href) {
        return XrefLink.regex.test(href);
    }
...
class UrlLink extends Link {
    ...
    async resolve(catalog) {
        return checkUrlExists(this.href);
    }
}
...
module.exports.RemoteImage = RemoteImage;
