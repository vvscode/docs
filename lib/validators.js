...
const { Page, Heading, XrefLink, UrlLink, MailtoLink, RemoteImage, LocalImage } = require('./catalog');
...
class XrefLinkValidator extends LinkValidator {
    constructor() {
        super(XrefLink);
    }
...
class MailtoLinkValidator extends LinkValidator {
    constructor() {
        super(MailtoLink);
    }
...
class RemoteHrefValidator extends LinkValidator {
    async resolve(link) {
...
class LocalHrefValidator extends LinkValidator {
    async resolve(link, catalog, options) {
        const page = link.page;
        const fileLocation = path.resolve(options.dir, page.directory, link.href);
...
module.exports = {
    ...
    urls: new RemoteHrefValidator(UrlLink),
    mailtos: new MailtoLinkValidator(),
    localimages: new LocalHrefValidator(LocalImage),
    remoteimages: new RemoteHrefValidator(RemoteImage),
